...
SERVER_HEALTH=$(curl -s -o /dev/null -w "%{http_code}" http://localhost:3002/api/clients/bulk-upload)
...
UPLOAD_RESPONSE=$(curl -s -X POST http://localhost:3002/api/clients/bulk-upload \
