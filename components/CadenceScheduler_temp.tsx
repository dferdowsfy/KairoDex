...
function alignToMonthDay(d: Date, dom: number) { 
  const x = new Date(d); 
  const last = lastDayOfMonth(x.getFullYear(), x.getMonth()); 
  x.setDate(Math.min(dom, last)); 
  if (x < d) { // move to next month if already past
    x.setMonth(x.getMonth() + 1); 
    const nextLast = lastDayOfMonth(x.getFullYear(), x.getMonth()); 
    x.setDate(Math.min(dom, nextLast));
  }
  return x 
}
...
export default function CadenceScheduler({
  ...
  const [cadenceType, setCadenceType] = useState<Cadence>(initial?.cadence || 'weekly')
  ...
  const [dayOfWeek, setDayOfWeek] = useState<number>(initial?.weekday ?? now.getDay())
  ...
  const [unitOfTime, setUnitOfTime] = useState<Unit>(initial?.customEvery?.unit || 'weeks')
  ...
  const preview = useMemo(() => {
    ...
    switch (cadenceType) {
      case 'weekly':
      case 'biweekly': {
        const stepWeeks = cadenceType === 'biweekly' ? 2 : 1
        let first = alignToWeekday(base, dayOfWeek)
      ...
      case 'quarterly': {
        const stepMonths = cadenceType === 'monthly' ? 1 : cadenceType === 'every_other_month' ? 2 : 3
    ...
    return dates
  }, [cadenceType, count, startDate, time, dayOfWeek, monthDay, customN, unitOfTime, maxPreview])
  ...
  useEffect(() => {
    ...
    if (initial?.cadence && initial.cadence !== cadenceType) setCadenceType(initial.cadence)
          ...
          <div className="flex gap-2 flex-wrap">
            {[
              ...
              <button
                key={opt.key}
                type="button"
                aria-pressed={cadenceType === (opt.key as Cadence)}
                onClick={() => {
                  if (DEBUG) console.log('Cadence option clicked:', opt.key)
                  setCadenceType(opt.key as Cadence)
                }}
                className={`px-3 py-2 rounded-lg text-sm border transition-colors ${cadenceType === (opt.key as Cadence) ? 'bg-slate-900 text-white border-slate-900' : 'bg-white text-slate-900 border-slate-200 hover:border-slate-300'}`}>
                {opt.label}
              </button>
            ))}
          </div>
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-3">
            {['Sun','Mon','Tue','Wed','Thu','Fri','Sat'].map((d, i) => (
              <button key={i} type="button" onClick={()=>{ if (DEBUG) console.log('Weekday clicked:', i); setDayOfWeek(i) }}
                className={`px-4 py-2 rounded-full border text-sm transition-colors ${dayOfWeek===i?'bg-slate-900 text-white border-slate-900':'bg-white text-slate-900 border-slate-200 hover:border-slate-300'}`}>{d}</button>
            ))}
          </div>
        </div>
      )}

      {['monthly','every_other_month','quarterly','custom'].includes(cadenceType) && (
          ...
          {cadenceType==='custom' && (
            <>
              ...
              <div>
                ...
                <select value={unitOfTime} onChange={e=>setUnitOfTime(e.target.value as Unit)} className="h-10 w-full input-neon px-3 text-sm">
